...
- (void)setStyleIndex:(NSInteger)index
{
    ...
    if (![picCountFlag isEqualToString:@""]) {
        _styleFileName = [NSString stringWithFormat:@"number_%@_style_%ld.plist",picCountFlag,(long)_styleIndex];
        _styleDict = nil;
...
- (void)buttonLongPressed:(UILongPressGestureRecognizer *)sender
{
    ...
    MeituImageEditView *scroll = (MeituImageEditView *)sender.view;
    NSLog(@"scroll.tag::%ld",(long)scroll.tag);
    if (sender.state == UIGestureRecognizerStateBegan)
    {
        startPoint = [sender locationInView:sender.view];
        originPoint = scroll.center;
        [self bringSubviewToFront:scroll];
        [UIView animateWithDuration:Duration animations:^{
            
            scroll.transform = CGAffineTransformMakeScale(1.1, 1.1);
            scroll.alpha = 0.7;
        }];
    }
    else if (sender.state == UIGestureRecognizerStateChanged)
    {
        ...
        scroll.center = CGPointMake(scroll.center.x+deltaX,scroll.center.y+deltaY);
        //NSLog(@"center = %@",NSStringFromCGPoint(scroll.center));
        NSInteger index = [self indexOfPoint:scroll.center withButton:scroll];
        NSLog(@"===%ld",(long)index);
...
//                contain = YES;
    ...
    else if (sender.state == UIGestureRecognizerStateEnded)
    {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            if (!contain)
            {
//                scroll.center = originPoint;
                if (_tempView) {
                    [self exchangeFromIndex:scroll.tag-1 toIndex:_tempView.tag-1];
                }else{
                
                    [scroll setNotReloadFrame:scroll.oldRect];
                }
            }
            _tempView = nil;
        }];
